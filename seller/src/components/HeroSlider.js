      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {slides.map((slide, index) => (
            <div
              key={index}
              className="w-full flex-shrink-0 flex items-center justify-between px-8 md:px-20"
            >
              {/* Left Side Text */}
              <div className="w-1/2 flex flex-col justify-center space-y-4 text-left pl-24">
                <h3 className="text-5xl md:text-6xl font-bold text-gray-800">
                  {typedBrand}
                  <span className="animate-pulse">|</span>
                </h3>
                <p className="text-xl md:text-2xl text-gray-600">{slide.desc}</p>

                {/* Offer & Discount */}
                <div className="space-y-1">
                  <p className="text-red-600 font-bold text-2xl animate-bounce">
                    {slide.offer}
                  </p>
                  <p className="text-green-600 text-xl italic animate-pulse">
                    {slide.discount}
                  </p>
                </div>

                {/* Buy Now Button */}
                <button className="bg-black text-white w-32 py-4 rounded-full shadow-md hover:scale-105 hover:bg-gray-800 transition-all duration-300">
                  Buy Now
                </button>
              </div>
